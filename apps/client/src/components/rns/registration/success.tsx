...
const RegistrationSuccess: React.FC<RegistrationSuccessProps> = ({
    ...
    const expiryDate = DateTime.now().plus({seconds: duration}).toFormat('MMMM d, yyyy');

    // Format the ETH and USD values
    const formattedEthPrice = formatEthValue(ethPrice);
    const formattedUsdPrice = formatUsdValue(usdPrice);
    const formattedTxFeeEth = formatEthValue(txFeeEth);
    const formattedTxFeeUsd = formatUsdValue(txFeeUsd);
            ...
            {showNotification && (
                <div className="fixed top-20 right-4 flex items-center gap-3 bg-white rounded-xl p-3 shadow-lg max-w-md z-50">
                    <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-600">Your &#34;Register name&#34; transaction was successful</p>
                    </div>
                    <a href="#" className="text-blue-500 text-xs whitespace-nowrap">View on Etherscan</a>
                </div>
            )}
            ...
            <div className="w-full max-w-md lg:max-w-2xl bg-white rounded-2xl shadow-md overflow-hidden">
                ...
                <div className="w-full text-center p-6 pt-8">
                    ...
                    <p className="text-base text-gray-700">
                        You are now the owner of <span className="text-blue-500">{trimmedDomainName(name)}</span>
                    </p>
                </div>
                    ...
                    <div className="w-full lg:w-72 border border-gray-100 rounded-xl">
                        <div className="grid grid-cols-1 divide-y divide-gray-100">
                            <div className="p-4 text-center sm:hidden">
                                <div className="flex justify-between">
                                    <div className="w-1/2 border-r border-gray-100">
                                        <div className="text-gray-500 text-xs mb-1">1 yr registration</div>
                                        <div className="font-medium text-base">{formattedEthPrice} ETH</div>
                                        <div className="text-xs text-gray-500">{formattedUsdPrice}</div>
                                    </div>
                                    <div className="w-1/2">
                                        <div className="text-gray-500 text-xs mb-1">Transaction fees</div>
                                        <div className="font-medium text-base">{formattedTxFeeEth} ETH</div>
                                        <div className="text-xs text-gray-500">{formattedTxFeeUsd}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-4 text-center">
                                ...
                                <button className="text-blue-500 text-xs flex items-center justify-center mx-auto mt-1 hidden">
