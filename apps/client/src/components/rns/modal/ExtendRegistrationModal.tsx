...
import {calculateNewExpiryDate, domainName, trimmedDomainName, formatDateTime} from '@/utils';
...
const ExtendRegistrationModal = ({
    ...
    const { renew, price, isPriceLoading } = useRenewal(ensName, duration, BigInt(currentExpiryDate));
    const { data: feeData } = useFeeData();
    const { ethPrice } = usePriceFeed();

    const gweiPrice = feeData ? Number(feeData.gasPrice) / 1e9 : null;
    const estimatedGasFee = 0.0003; // ETH
    ...
    const totalUsdPrice = totalEthPrice && ethPrice ? (totalEthPrice * Number(ethPrice)).toFixed(2) : '0';
    ...
    const StepOne = () => (
        <div className="space-y-6">
            <div className="flex justify-between items-center gap-2">
                <h2 className="text-sm sm:text-2xl font-bold text-gray-900 truncate">Extend {trimmedDomainName(ensName)}</h2>
                <button
                    onClick={onClose}
            ...
            {/* Duration Selector */}
            <div className="rounded-full bg-gray-100 p-1 flex items-center justify-between">
                <button
                    onClick={() => handleDurationChange(false)}
                    disabled={!minDuration || duration <= minDuration}
                    className="w-12 h-12 rounded-full flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white
            ...
            <div className="flex items-center justify-between text-gray-500 text-sm">
                ...
                <div className="flex gap-2">
                    <button
                        className={`px-3 py-1 rounded-lg ${currency === 'ETH' ? 'bg-blue-500 text-white' : 'bg-gray-100'}`}
            ...
            <div className="bg-gray-50 rounded-xl border p-4 space-y-2">
                <div className="flex justify-between text-gray-600">
                    ...
                    <span>
                        {currency === 'ETH'
                            ? formattedPrice ? `${formattedPrice} ETH` : 'Loading...'
                            : formattedPrice && ethPrice ? `$${(Number(formattedPrice) * Number(ethPrice)).toFixed(2)}` : 'Loading...'}
                    </span>
                </div>
                <div className="flex justify-between text-gray-600">
                    ...
                    <span>
                        {currency === 'ETH'
                            ? `${estimatedGasFee} ETH`
                            : ethPrice ? `$${(estimatedGasFee * Number(ethPrice)).toFixed(2)}` : 'Loading...'}
                    </span>
                </div>
                <div className="flex justify-between text-gray-900 font-medium pt-2 border-t border-gray-200">
                    ...
                    <span>
                        {currency === 'ETH'
                            ? totalEthPrice ? `${totalEthPrice.toFixed(4)} ETH` : 'Loading...'
                            : `$${totalUsdPrice}`}
                    </span>
                </div>
            </div>
            ...
            <div className="flex gap-3">
                <button
                    onClick={onClose}
                    className="flex-1 py-3 px-4 rounded-xl bg-blue-50 text-blue-500 hover:bg-blue-100
                ...
                <button
                    ...
                    className="flex-1 py-3 px-4 rounded-xl bg-blue-500 text-white hover:bg-blue-600
    ...
    const StepTwo = () => (
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                <div className="bg-gray-50 border rounded-lg p-4 flex justify-between items-start sm:items-center">
                    <span className="text-gray-500 flex-shrink-0">Name</span>
                    <div className="flex items-center gap-2 max-w-[70%]">
                        <p className="text-gray-900 text-right line-clamp-2 break-words break-all hyphens-auto text-xs sm:text-sm md:text-base">
                            {domainName(ensName)}
                        </p>
                        <div className="w-6 h-6 flex-shrink-0 rounded-full bg-gradient-to-br from-green-300 to-pink-300" />
                    </div>
                </div>
                ...
                <div className="bg-gray-50 border rounded-lg p-4 flex justify-between items-center">
                    ...
                    <span className="text-gray-900">
                        {totalEthPrice ? `${totalEthPrice.toFixed(4)} ETH + gas` : 'Loading...'}
                    </span>
                </div>
            </div>
            ...
            <div className="flex gap-3">
                ...
                >
                    ...
                    {isProcessing ? 'Processing...' : isPriceLoading ? 'Loading...' : 'Confirm Extension'}
                </button>
            </div>
        </div>
    );
